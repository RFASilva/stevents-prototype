...
package core.time_series.spatial_utilities.snn_bf.com.savarese.spatial;
  ...
  implements Point<Coord>
{
   ...
   * greater than 0.
   * @param allowsRep if the allows repetitions.
   */
  public GenericPoint(int dimensions, boolean allowsRep) {
    assert(dimensions > 0);
    __coordinates  = new Comparable[dimensions];
    setId(-1);
    setAllowsRep(allowsRep);
  }
   ...
   * @param allowsRep if the allows repetitions.
   */
  public GenericPoint(Coord x, Coord y, boolean allowsRep) {
    this(2, allowsRep);
    setCoord(0, x);
    setCoord(1, y);
  }
   ...
   * @param allowsRep if the allows repetitions.
   */
  public GenericPoint(Coord x, Coord y, Coord z, boolean allowsRep) {
    this(3, allowsRep);
    setCoord(0, x);
    setCoord(1, y);
    setCoord(2, z);
   ...
   * @param allowsRep if the allows repetitions.
   */
  public GenericPoint(Coord x, Coord y, Coord z, Coord w, boolean allowsRep) {
    this(4, allowsRep);
    setCoord(0, x);
    setCoord(1, y);
    setCoord(2, z);
  ...
  /**
   * Returns the value of the flag of handling repetitions.
   */
  public boolean isAllowsRep(){
	  return allowsRep;
  }
  
  /**
   * Sets the flag of handling repetitions.
   * 
   * @param the value of the flag (0, if it should eliminate repetitions. 
   * Otherwise, 1.).
   */
  public void setAllowsRep(boolean allowsRep){
  ...
  public Object clone() {
    GenericPoint<Coord> point =
      new GenericPoint<Coord>(__coordinates.length, allowsRep);
    for(int i = 0; i < __coordinates.length; ++i)
